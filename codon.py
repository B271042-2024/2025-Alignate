...
#_______________________________________________________________________________________________
    ...
    def show_tcsh_warning(self):
        ...
        msg.setText("Warning: Missing 'tcsh' shell. \nSome features like PSIPRED will not work.")
            ...
            if group['layout_seq'] == layout:
                ...
                for i in reversed(range(layout.count())):
                    ...
                    if widget and widget.objectName() in target_names:
                        ...
                        for name in target_names:
                            nested = widget.findChile(QWidget, name)
                            if nested:
...
# --------------------------------------------Main
        ...
        file_path = QFileDialog.getOpenFileName(self, 'Select File', 'FASTA files (*.fasta *.fa *.txt);;All Files (*)')
        if not file_path:
            return
            ...
            for idx_col, score in enumerate(similarity):
                for (group_idx, entry_idx) in seq_map:
                    ...
                    if idx_col < len(entry['seq_letters']):
                        lbl = entry['seq_letters'][idx_col]                             # grab letters from dict by using column index
# --------------------------------------------Connect: inner DEF 2
                        lbl.setProperty('bg_color', similarity_to_color(score))         # make a dynamic property global - can access later by object.property('bg_color'). setObjectName is static.
