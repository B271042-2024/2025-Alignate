...
from PySide6.QtWidgets import QFrame, QGroupBox, QRadioButton, QStackedWidget, QProgressBar, QFileDialog, QMessageBox, QDialog, QTextEdit, QDialogButtonBox, QLayout, QScrollArea, QSizePolicy, QApplication, QMainWindow, QWidget, QCheckBox, QLabel, QLineEdit, QPushButton, QHBoxLayout, QVBoxLayout, QSlider
...
# --------------------------------------------Sub-elements
        ...
        self.checkboxslider.setToolTip('Check to activate slider')
...
# --------------------------------------------Connect        
                for group in self.groups:
                    if group['layout_seq'] == layout:
                        ...
                        # calculate the no of bases
                        # if no of bases is > 10 bases different, warning
...
#__________________________________________________________________________________________ALIGN
    ...
    def get_mafft_path(self):
        ...
        if sys.platform.startswith('linux'):                                              # linux
            ...
            QMessageBox.critical(self, 'Error', f'Unsupported OS')
...
#           ------------------------------- Connect: Widget Progress 5 -------------------------------
                ...
                QMessageBox.critical(self, 'Error', f'PSIPRED failed: {e}')
...
# -------------------------- for aligned reference sequence ---------------------------
        ...
        print(f'initial pred:\n', pred)
        print(f'initial final_pred:\n', final_pred)
 ...
 # -------------------------- for aligned reference sequence ---------------------------
        ...
        while i < len(final_pred):
            ...
            if ss == 'H':
                ...
                troubleshoot.append(f'H: {start}, {end}')
            elif ss == 'E':
...
# -------------------------- end: for aligned reference sequence ---------------------------
    ...
    # --------------------------------------------Action
        ...
        for ss_type, start, end in regions:                                                                                 # C/H/E, start_pos, end_pos
            match_count = sum(
                1 for i in range(start, end + 1)
                if i < len(target_cons) and i < len(ref_cons) and target_cons[i] == ref_cons[i]                             # at position i, if consensus target = ref, add 1
            )
            total = end - start + 1                                                                                         # total length at C/H/E
            percent = (match_count / total) * 100 if total else 0                                                           # calculate percentage match
            mid = (start + end + 1) // 2                                                                                        # position at the middle
            if mid < len(labels):
